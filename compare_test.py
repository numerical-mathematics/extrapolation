...
for i in range(len(tol)):
    ...
    y, infodict = ex_p.ex_midpoint_parallel(f, y0, [t0, tf], atol=tol[i], rtol=tol[i], adaptive="order", full_output=True)
    ...
    start_time = time.time()
    r = ode(f2).set_integrator('dopri5', atol=tol[i], rtol=tol[i], verbosity=10, nsteps= 10e5)
    ...
    start_time = time.time()
    r = ode(f2, jac=None).set_integrator('dop853', atol=tol[i], rtol=tol[i], verbosity=10, nsteps= 10e5)
    r.set_initial_value(y0, t0)
    r.integrate(r.t+(tf-t0))
    assert r.t == tf, "Integration did not converge. Try increasing the max number of steps"
