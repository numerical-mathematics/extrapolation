...
def _solve_implicit_step(zero_f, zero_grad, estimatedValue, addSolverParam):
    """
    Find the root of the zero_f function using as initial approximation
    estimatedValueExplicit.  This zero_f root is the next step solution. The
    _solve_implicit_step solves one step of an implicit method.
    ...
    # TODO: pass to the zero finder solver the tolerance required to the overall
    # problem 
    # by doing this the error by the solver doesn't limit the global error of
    # the ODE's solution
    ...
    x, infodict, _, _ = optimize.fsolve(zero_f, estimatedValue, 
...
def _midpoint_semiimplicit(f, grad, previous_values, previousTime,
                           ...
                           Isparse):
    """
    Calculates solution at previousTime+step doing one step with a midpoint
...
def _euler_semiimplicit(f, grad, previous_values, previousTime, 
                        ...
                        Isparse):
    """
    Calculates solution at previousTime+step doing one step with a euler
    semiimplicit formula (linearly implicit euler) Based on IV.9.25 (ref II).
...
def _midpoint_implicit(f, grad, previous_values, previousTime, f_previousValue,
    ...
    def zero_func(x):
        fval = f(*((previousValue+x)/2, previousTime+step/2) + args)
        return x - previousValue - step * fval
...
def _compute_stages((method, methodargs, func, grad, tn, yn, f_yn, args, h,
    ...
    for (j,nj) in j_nj_list:
        ...
        # needed (interpolation) extract from Y
        y_half = Y[nj/2]
...
def _compute_extrapolation_table(method, methodargs, func, grad, tn, yn, args,
    ...
    @param method (callable(...)): the method on which the extrapolation is
           based (euler,mipoint/explicit, implicit,semiimplicit)
    @param methodargs (dict): dictionary with extra parameters to be passed to
           the solver method.
    ...
    @param rejectPreviousStep (bool): whether previously taken step was
           rejected or not 
    @param previousStepSolution (2-tuple): tuple containing the solution at the
    ...
    @param smoothing (string): specifies if a smoothing step should be
           performed:
            ...
            -'semiimp': two point smoothing step (for semiimplicit midpoint),
             IV.9.16c ref II. 
    @param symmetric (bool): whether the method to solve one step is symmetric
           (midpoint/trapezoidal) or non-symmetric (euler).
    @param addSolverParam (dict): extra arguments needed to define completely
           the solver's behavior.  Should not be empty, for more information see
           _getAdditionalSolverParameters(..) function.
...
def _interpolate_sym(y0, Tkk, f_Tkk, y_half, f_yj, hs, H, k, atol, rtol,
                     seq=(lambda t: 4*t-2)):
    """
    Symmetrical formula (for example used for midpoint's method) to interpolate
...
def _solve_one_step(method, methodargs, func, grad, t_curr, t, t_index, yn,
    ...
    if((not rejectStep) & dense):
        rejectStep, y_solution, h_int, (fe_tot_, fe_seq_) = _interpolate_values_at_t(func, 
...
def _interpolate_values_at_t(func, args, T, k, t_curr, t, t_index, h, hs,
...
"""
BEGINNING BLOCK 2: General extrapolation solver functions. These functions
can be used to solve any ODE.  These are recommended to use for developing and
testing purposes.
    
Each function solves the system of IVPs dy/dt = func(y, t0, ...) with parallel
extrapolation.

Important: the default values of the functions are set to achieve the optimal
performance, it is highly recommended to use such default values (this applies
to robustness, adaptive, seq and smoothing parameters).

All this functions have the same structure, and their names explicitly say the
type of solver method used (midpoint,euler/explicit,semiimplicit,implicit).
Structure:
...
def ex_midpoint_explicit_parallel(func, grad, y0, t, args=(), full_output=0,
                                  rtol=1.0e-8, atol=1.0e-8, h0=0.5,
                                  mxstep=10e4, robustness=2, smoothing = 'no',
                                  seq=(lambda t: 2*t), p=4, nworkers=None,
                                  adaptive="order"):
    """
...
def ex_midpoint_implicit_parallel(func, grad, y0, t, args=(), full_output=0,
                                  rtol=1.0e-8, atol=1.0e-8, h0=0.5,
                                  mxstep=10e4, robustness=2, smoothing = 'gbs',
                                  seq=(lambda t: 2*(2*t-1)), p=4,
...
"""
END BLOCK 2: General extrapolation solvers' functions. These functions can be
used to solve any ODE.  These are the user interface functions.
"""
    
"""
USER INTERFACE Main Function: ODE solver based on extrapolation methods based
on II.9 ref I (non stiff problems) and IV.9 ref II (stiff problems).

General function similar to BLOCK 2 functions, with less parameters, thus for a
general user. To develop and test use BLOCK 2 functions as those functions have
more parameters to be chosen (if this functions is used such parameters are set
to the 'optimal').
...
def extrapolation_parallel(method_name, func, grad, y0, t, args=(),
                           full_output=0, rtol=1.0e-8, atol=1.0e-8, h0=0.5,
                           mxstep=10e4, p=4, nworkers=None, adaptive =
                           'order'):
